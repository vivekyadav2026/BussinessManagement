...
<div class="dash-head flex justify-between items-end mb-6">
    ...
    <div class="flex gap-2">
        <a href="{{ route('organization.attendance.report') }}" class="btn border border-gray-300 text-gray-700 bg-white hover:bg-gray-50 btn-sm">Monthly Report</a>
    </div>
</div>

<div class="panel mb-6 bg-gray-50">
    <form method="GET" action="{{ route('organization.attendance.index') }}" class="flex gap-4 items-end">
        ...
        <a href="{{ route('organization.attendance.index') }}" class="text-indigo-600 hover:underline text-sm pb-2">Jump to Today</a>
        @endif
    </form>
</div>
...
<form action="{{ route('organization.attendance.storeBulk') }}" method="POST">
    ...
    <div class="panel p-0 overflow-hidden">
        <table class="w-full text-left">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    <th class="p-4 text-xs font-bold text-gray-500 uppercase">Employee</th>
                    <th class="p-4 text-xs font-bold text-gray-500 uppercase">Status</th>
                    <th class="p-4 text-xs font-bold text-gray-500 uppercase">Check In</th>
                    <th class="p-4 text-xs font-bold text-gray-500 uppercase">Check Out</th>
                    <th class="p-4 text-xs font-bold text-gray-500 uppercase text-right">History</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="p-4">
                        ...
                        <div class="text-xs text-gray-500">{{ $emp->employee_code ?? 'No Code' }} • {{ $emp->designation ?? 'Staff' }}</div>
                    </td>
                    <td class="p-4">
                    ...
                    <td class="p-4 text-right">
                        <a href="{{ route('organization.attendance.show', $emp) }}" class="text-indigo-600 hover:underline text-sm font-medium">Calendar</a>
                    </td>
                </tr>
                @empty
    ...
    <div class="mt-6 flex justify-end">
        <button type="submit" class="btn btn-gold py-2 px-8 shadow-sm">Save Attendance</button>
    </div>
    @endif
</form>
@endsection
