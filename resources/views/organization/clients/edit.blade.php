...
<div class="max-w-4xl mx-auto space-y-6 pt-2 sm:pt-4 pb-20">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div class="min-w-0 flex-1">
            <nav class="flex items-center gap-2 text-xs font-semibold text-slate-600 mb-2" aria-label="Breadcrumb">
                ...
                <span class="text-slate-400 font-bold">/</span>
                <span class="text-slate-950 font-extrabold truncate max-w-[200px]">Edit: {{ $client->name }}</span>
            </nav>
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-950 tracking-tight">Edit Client: {{ $client->name }}</h1>
                    <p class="text-xs sm:text-sm text-slate-600 font-medium mt-0.5">
                        Update customer profile attributes, GST credentials, or active billing permissions.
                    </p>
                </div>
            </div>
        </div>
    ...
    <!-- 2. Form Card -->
    <form action="{{ route('organization.clients.update', $client) }}" method="POST" class="bg-white rounded-xl border border-slate-200/90 shadow-2xs overflow-hidden">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 md:p-8 space-y-6">
            ...
            <div class="flex items-center gap-2.5 pb-3 border-b border-slate-100">
                ...
                <h2 class="text-xs font-extrabold text-slate-950 uppercase tracking-wider">Modify Client Parameters</h2>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="col-span-full">
                    ...
                    <textarea name="address" rows="2" 
                        ...
                        placeholder="e.g. Plot No. 12, Industrial Area, Sector 4, City, State - PIN">{{ old('address', $client->address) }}</textarea>
                ...
                <div class="col-span-full">
                    ...
                    <textarea name="notes" rows="2" 
                        ...
                        class="w-full border border-slate-300 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 rounded-lg px-3.5 py-2.5 text-xs sm:text-sm font-medium outline-none transition text-slate-950 placeholder:text-slate-400 shadow-2xs @error('notes') border-rose-300 @enderror">{{ old('notes', $client->notes) }}</textarea>
                ...
                <div class="col-span-full pt-2">
                    <label class="flex items-center gap-3 cursor-pointer select-none">
                        ...
                        <div>
                            <span class="text-xs font-extrabold text-slate-900 block">Account Active for Invoicing</span>
                            <span class="text-[11px] text-slate-500 font-medium">When inactive, this client cannot be selected for new billing tickets.</span>
                        </div>
                    </label>
                </div>
            </div>
