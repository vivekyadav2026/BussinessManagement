<x-guest-layout maxWidth="max-w-md">
    <x-slot name="header">
        <h2 class="text-center text-xl sm:text-2xl font-bold tracking-tight text-gray-900">Sign in to your account</h2>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-4">
        ...
        <div>
            ...
            <div class="relative rounded-xl shadow-2xs">
                <input id="email" name="email" type="email" autocomplete="email" required autofocus class="block w-full appearance-none rounded-xl border border-gray-300 px-3.5 py-2.5 placeholder-gray-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 text-sm transition" value="{{ old('email') }}" placeholder="name@company.com">
            </div>
        ...
        <div x-data="{ show: false }">
            ...
            <div class="relative rounded-xl shadow-2xs">
                <input id="password" name="password" :type="show ? 'text' : 'password'" autocomplete="current-password" required class="block w-full appearance-none rounded-xl border border-gray-300 px-3.5 py-2.5 pr-10 placeholder-gray-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 text-sm transition" placeholder="••••••••">
        ...
        <div class="flex items-center">
            <input id="remember_me" name="remember" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer">
            <label for="remember_me" class="ml-2 block text-xs font-semibold text-gray-700 cursor-pointer">Remember me on this device</label>
        </div>
