@php
    $isAdmin = auth()->user()->hasRole('Organization Admin');
    $can = fn($permission) => $isAdmin || auth()->user()->hasPermissionTo($permission);
    $feature = fn($code) => \App\Services\SubscriptionService::hasFeature(auth()->user()->organization_id, $code);
@endphp
...
@if($isAdmin || $can('employees.view') || $can('roles.view') || ($isAdmin && $feature('multi_location')))
    <div x-show="typeof sidebarCollapsed === 'undefined' || !sidebarCollapsed" class="mt-4 mb-2 px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Administration</div>
@endif
...
@if($feature('module_retail'))
    <!-- Operations -->
    @if($can('inventory.view') || $can('clients.view') || $can('invoices.view'))
        <div x-show="typeof sidebarCollapsed === 'undefined' || !sidebarCollapsed" class="mt-4 mb-2 px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Operations</div>
    @endif
    ...
    @if($can('products.view'))
        ...
        <x-sidebar-item route="{{ route('organization.categories.index') }}" icon='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>' :active="request()->routeIs('organization.categories.*')">
            Product Categories
        </x-sidebar-item>
    @endif
    ...
    @if($can('invoices.view'))
        <x-sidebar-item route="{{ route('organization.invoices.index') }}" icon='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>' :active="request()->routeIs('organization.invoices.*')">
            Invoices
        </x-sidebar-item>
        <x-sidebar-item route="{{ route('organization.receivables.index') }}" icon='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>' :active="request()->routeIs('organization.receivables.*')">
            Receivables
        </x-sidebar-item>
        <x-sidebar-item route="{{ route('organization.sales-analytics.index') }}" icon='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 012-2h2a2 2 0 012 2v6m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>' :active="request()->routeIs('organization.sales-analytics.*')">
            Sales & Analytics
        </x-sidebar-item>
    @endif
@endif
...
@if($feature('module_payroll'))
    @if($can('attendance.view') || $can('payroll.view'))
        <div x-show="typeof sidebarCollapsed === 'undefined' || !sidebarCollapsed" class="mt-4 mb-2 px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">HR & Payroll</div>
    @endif
    ...
    @if($can('payroll.view'))
        <x-sidebar-item route="{{ route('organization.payroll.index') }}" icon='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>' :active="request()->routeIs('organization.payroll.*')">
            Payroll
        </x-sidebar-item>
    @endif
@endif
...
@if($can('complaints.view'))
    <x-sidebar-item route="{{ route('organization.complaints.index') }}" icon='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>' :active="request()->routeIs('organization.complaints.*')">
        Complaints
    </x-sidebar-item>
@endif
...
@if($feature('module_restaurant'))
    @if($can('restaurant.view'))
        <div x-show="typeof sidebarCollapsed === 'undefined' || !sidebarCollapsed" class="mt-4 mb-2 px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Restaurant</div>
    @endif
    ...
    @if($isAdmin || $can('restaurant.view') || $can('restaurant.orders') || $can('restaurant.menu') || $can('restaurant.tables') || $can('restaurant.kitchen') || $can('restaurant.counter'))
        @if($isAdmin || $can('restaurant.view'))
            <x-sidebar-item route="{{ route('organization.menu.pos.index') }}" icon='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>' :active="request()->routeIs('organization.menu.pos.*')">
                Waiter POS & Billing
            </x-sidebar-item>
        @endif
        
        @if($isAdmin || $can('restaurant.counter'))
            <x-sidebar-item route="{{ route('organization.menu.counter.index') }}" icon='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/></svg>' :active="request()->routeIs('organization.menu.counter.*')">
                Counter Billing
            </x-sidebar-item>
        @endif
        
        @if($isAdmin || $can('restaurant.kitchen'))
            <x-sidebar-item route="{{ route('organization.menu.kitchen.index') }}" target="_blank" icon='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>' :active="request()->routeIs('organization.menu.kitchen.index')">
                Kitchen Display
            </x-sidebar-item>
        @endif
        
        @if($isAdmin || $can('restaurant.reports'))
            <x-sidebar-item route="{{ route('organization.menu.reports.index') }}" icon='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 012-2h2a2 2 0 012 2v6m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>' :active="request()->routeIs('organization.menu.reports.*')">
                Restro Sales & Analytics
            </x-sidebar-item>
        @endif
    @endif
@endif
...
<!-- System -->
@if($isAdmin)
    <div x-show="typeof sidebarCollapsed === 'undefined' || !sidebarCollapsed" class="mt-4 mb-2 px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">System</div>
@endif
